  ...
  <div className='grid'>
    ...
    {filteredcards.map((card) => (
      <div className='card' key={card.id} onClick={() => this.showPanel(card)}>
        ...
        <h3>{card.title}</h3>
        {/* Corrected typo: "desription" to "description" */}
        <p>{card.description}</p>
      </div>
    ))}
  </div>
  {/* - Moved the sliding panel outside of the .map() loop.
    - Corrected typo: "slectedcard" to "selectedcard".
  */}
  {selectedcard && (
    <div className='overlay'>
      <div className='slidingpanel'>
        ...
        {/* You can add more details from selectedcard here */}
      </div>
    </div>
  )}
</section>
